...
\ea%2
    ...
    \gll    *Paul   me       lui       présentera.\\
            \hspaceThis{*}she   me.\textsc{acc}=   him.\textsc{dat=} present.\textsc{3sg.fut}\\
    \glt    Intended: ‘Paul will introduce me to him.’\todo{The use of ='s is inconsistent with Leipzig Glossing Rules (LGR) throughout the file}
\z

The presence of \REF{ex:sheehan:1b} is seemingly crucial to the definition of the PCC because the effect disappears, in ditransitives, where the indirect object is a non-clitic (\citealt{Kayne1975, Rezac2008}). The meaning intended to be conveyed by \REF{ex:sheehan:2} can easily be conveyed using an unfocussed tonic pronoun introduced by à, which is exceptionally allowed in such contexts:\footnote{I gloss \textit{a/à} as ‘to’ throughout for expositional purposes, but one of the main claims of this paper is that sometime this morpheme is a realisation of dative case marking and at other times it is a locative preposition.} 
...
\ea%10
    ...
    \gll    Gaizki   irudi-tzen ${\emptyset}$-zai-${\emptyset}$-t         [zuk ni harakin-ari    saltze-a]\\
          wrong   look-\textsc{ipfv} \textsc{3sg}-be-\textsc{sg}.\textsc{abs}{}-\textsc{1sg}.\textsc{dat}  \hspaceThis{[}you.\textsc{erg}    me.\textsc{abs} butcher-\textsc{art.sg}.\textsc{dat} sell-\textsc{nmlz-art.sg.abs}\\
    \glt 'It seems wrong to me [for you to sell me to the butcher.]'\todo{\textsc{impf} updated as \textsc{ipfv} per LGR, I suggest updating the \textsc{art}'s as \textsc{def}}\todo{The morphemes in the sentence-final word do not correspond}
\z
...
\ea%11
    ...
    \glt ‘They sent Mateo/your son to the doctors’\todo{Peter was changed to Mateo}
...
\ea%13
    ...
    \gll    *Je   vous     lui           laisserai   voir\\
            \hspaceThis{*}I      you.\textsc{acc}=   her.\textsc{dat=}   let.3\textsc{sg}.\textsc{fut}    see\\
...
\ea%14
    ...
    \gll    *Marcel vous   a   fait   épouser   au     médecin.\\
            \hspaceThis{*}Marcel   you.\textsc{acc}=  has   made   marry   to.the   doctor\\
    ...
    \gll    *On   vous   laissera   connaître   à Louise.\\
            \hspaceThis{*}one   you\textsc{.acc}=  let.\textsc{3sg.fut} know     to Louise\\
...
Postal calls this the ‘Fancy Constraint’ and perhaps for this reason it is not usually discussed in connection with the PCC. It is, however, essentially a simpler version of the PCC, which we will call the ‘Simpler PCC’\todo{Single quotation markers are for linguistic meaning only}:
...
\ea%17
    ...
    \gll   *Em       van     fer     escollir   a   la   Teresa\\
            \hspaceThis{*}me.\textsc{acc}=  go.\textsc{3pl}   make   choose   to   the   Teresa\\
...
\ea%18
    ...
    \gll   *Ti     ho     fatto   picchiare     a   mio   fratello\\
         \hspaceThis{*}you.\textsc{acc}  have.\textsc{1sg}   made beat         to   my   brother\\
...
This is further potential evidence that we are dealing with the PCC. Though the structure of the \textit{faire-par} construction remains contested, there is widespread recognition that the ‘by phrase’ in examples like \REF{ex:sheehan:19a} has adjunct-like properties and is not even projected in \REF{ex:sheehan:19b} (see \citealt{Guasti1996, FolliHarley2007, SheehanCyrino2016} for recent discussion). In any case, evidence from binding shows that a by-phrase causee does not c-command the accusative object in the \textit{faire-par} construction, whereas a dative causee in the \textit{faire-infinitive} does, as \citet{Burzio1986} shows:
...
\ea%20
    ...
    \glt    ‘I made Gianni repair his own car.’\todo{The optionality is not really clear; \textsc{1s} -> \textsc{1sg} per LGR and many \textsc{3s} corrections throughout the file}
\z
...
\ea%21
    ...
    \gll    Ho   fatto   leggere [ogni   libro]\textsubscript{i} dal   suo\textsubscript{i}   autore.\\
            have.\textsc{1sg}   made   read   \hspaceThis{[}each book   by.the   its   author\\
    ...
    \gll    *Ho   fatto   leggere   il  suo\textsubscript{i} libro   da   [ogni  autore]\textsubscript{i}\\
            \hspaceThis{*}have.\textsc{1sg}   made   read   the  his   book   by   \hspaceThis{[}each   author\\
    \z
\z
...
There is a clear contrast when we consider examples with 1st/2nd person direct object and a 3\textsuperscript{rd} person causee, the ‘strong PCC’ context. These were most unacceptable with dative clitics \REF{ex:sheehan:25a}, but were also rated very low with full DP datives (an average of around 2 on the scale 8-point scale) \REF{ex:sheehan:25b}:
...
\ea%25
    ...
    \gll    *Le professeur  \textbf{me/te}       \textbf{lui}   fait   voir.\\
            \hspaceThis{*}the   teacher  me.\textsc{acc/}you.\textsc{acc}=  him.\textsc{dat}=  make see\\
    ...
    \gll    *?La   professeure   \textbf{me/te}       fait     voir à   Marie,  qui   se  sent   à   l’  aise. \\
            \hspaceThis{*?}the     teacher     me.\textsc{acc}/you.\textsc{acc=} makes   see to Marie, who \textsc{se}   feels   at   the ease\\
    \glt    Intended: ‘The teacher shows you to Marie, who is feeling at ease.’\todo{`me' is not reflected in the translation in (b)}\\
...
ECM is not usually possible with Italian FARE (but see \citealt{Burzio1986, SchifanoSheehan2017} for discussion). This repair is not possible with full DP causees, for unclear reasons, making it only partially parallel to what is described for Spanish below.}\\
    \gll    *Ti ho        fatto   picchiare   a    lui/LUI\\
            \hspaceThis{*}you have   made   beat     to him/HIM\\
...
According to \citet{Torrego2010}, clitic doubling of datives in the faire-infinitive is optional, at least for some Spanish speakers (see also \citealt{Pineda2013} regarding ditransitives). I take the VS order in \REF{ex:sheehan:29} to indicate that this is an instance of the \textit{faire infinitive} nonetheless:
...
\ea%30
    ...
    \gll    *Marcelo   se   te   hizo   saludar  al   invitado.\\
            \hspaceThis{*}Marcelo   him.\textsc{dat}=  you.\textsc{acc}=  made   greet  to.the   guest\\
...
\ea%32
    ...
    \gll    Él  hizo  [a   Sadat  exportar=las      desde   Francia].\\
            He  made    \hspaceThis{[}to   Sadat  export.\textsc{inf}=them.\textsc{acc}   from   France\\
    \ex\label{ex:sheehan:32c}
    \gll    *Él  las  hizo [a   Sadat   exportar   desde   Francia].\\
            \hspaceThis{*}he them.\textsc{acc}=   made   \hspaceThis{[}to Sadat   export.\textsc{inf} from   France\\
    \z
\z\todo{Per LGR, "exportar" has been changed to "expor=tar"}
...
\ea%33
    ...
    \gll    *El   me     lo     hizo   saludar.\\
            \hspaceThis{*}he   me.\textsc{dat}=  him.\textsc{acc}=  made greet\\
    \glt    ‘He made me greet him.’\todo{saludarlo -> saludar=lo}
...
\ea%34
    ...
    \gll    (?)Marcelo   hizo   al   invitado  saludar=te.\\
            \hspaceThis{(?)}Marcelo   made  to.the guest greet=you.\textsc{acc}\\
    \glt    \textsc{‘}Marcelo made the guest greet you.’\todo{saludar -> saludar=te}
    \ex\label{ex:sheehan:34b}
    \gll    (?)Dejará   a   Luisa   vernos.\\
            \hspaceThis{(?)}let.\textsc{fut}   to   Luisa   see=us.\textsc{acc}\\
    \glt    ‘They will let Luisa see us.’\todo{Please update "vernos" accordingly}
    \z
\z
...
\ea%36
    ...
    \gll    \%Marcelo   le   hizo   al   invitado saludar=te.\\
            \hspaceThis{\%}Marcelo   him.\textsc{dat=}   made   to.the   guest  greet=you.\textsc{acc}\\
...
In other words, it is this structural ambiguity in ditransitives which gives rise the false impression that the PCC only holds with dative clitics. Full DPs introduced by \textit{a/à} which occur with ditransitive verbs can be either dative or locative, having either the structure in \REF{ex:sheehan:40a} or that in \REF{ex:sheehan:40b}, whereas dative clitics are unambiguously dative, and so must derive from the structure in \REF{ex:sheehan:40a}\footnote{For concreteness, I assume that clitics originate in argument positions, but there are other possibilities.} . Consider, by way of illustration, the French examples in \REF{ex:sheehan:2}-\REF{ex:sheehan:3} above, repeated here as (\ref{ex:sheehan:41a}-b): 
...
\ea%41
    ...
    \gll    *Paul   me       lui       présentera.\\
            \hspaceThis{*}Paul   me.\textsc{acc}=   him.\textsc{dat=} present.\textsc{3sg.fut}\\
...
\ea%42
    ...
    \gll  \%Paul   m’       y       présentera.\\
            \hspaceThis{\%}Paul   me.\textsc{acc}=   there\textsc{=}   present.\textsc{3sg.fut}\\
    \glt   ‘Paul will introduce me to him.’
\z
...
\ea%43
    ...
    \gll    Elle m'   a   (*tous)   présentée     à   eux.\\
          she me.\textsc{acc}= has   \hspaceThis{(*}all     introduced  to   them\\
    \glt     ‘She has introduced me to (*all of) them.’
    \z
\z

Example \REF{ex:sheehan:43a} shows that cliticization permits quantifier float. The fact that this is not possible in \REF{ex:sheehan:43b} follows if this repair involves a different base generated structure, rather than a PF repair. 
