...
\subsection{Preposition change in BP}\label{sec:calindro:2.1}\todo{section 2.1. is orphaned and starts to late}
...
Therefore, the two prepositions \textit{a} and \textit{para} in BP share the same semantic status, indicating that \textit{a} is no longer a dative marker as it is in EP DOCs (cf. \ref{ex:calindro:3} and 67\todo{what is 67 referring to?}). Therefore, a Brazilian child acquiring language in this context does not access this semantic difference shown in \REF{ex:calindro:12} for EP.
