...
The same analysis can be applied to Spanish and Portuguese. The examples in (\ref{ex:cepeda:11}) suggest that the Spanish and Portuguese sentences in (\ref{ex:cepeda:9}) are really non-idiomatic since \textit{lata} and \textit{canja} can be interpreted as \todo{Single quotation markers are for linguistic meaning only}‘bothersome situation’ (\ref{ex:cepeda:11a}) and ‘easy situation’ (\ref{ex:cepeda:11b}), respectively, even without the presence of the verb.
...
\ea%i
	...
	\ex\label{ex:cepeda:ib} \hspaceThis{?/\#}Olga gave [$_{IO}$ to Mario] [$_{DO}$ a reason not to accept the job].
	\z
\z
...
\ea%24
	...
	\gll {?/\#}A Olga deu [$_{DO}$  três razões para não aceitar o trabalho] [$_{IO}$ ao Mario]\\
		\hspaceThis{?/\#}the Olga gave \hspaceThis{[$_{DO}$} three reasons to not accept the job \hspaceThis{[$_{IO}$} to.the Mario\\
...
\ea \label{ex:cepeda:28}
	\gll A   Olga deu $[_{DO}$ uma maçã] $[_{IO}$ para o   Mario].\\
			the Olga gave \hspaceThis{$[_{DO}$} an apple \hspaceThis{$[_{IO}$} to the Mario\\
	\glt ‘Olga gave an apple to Mario.’
		\ea[]{[$_{TP}$ A Olga  T+\textit{v}+deu [$_{\textit{v}P}$ \textcolor{gray}{<A Olga> <\textit{v}+deu>} [$_{VP}$ [uma maçã] \textcolor{gray}{<deu>} [para o Mario] ] ] ]}\label{ex:cepeda:28a}
		\ex[]{[$_{TP}$ A Olga T+\textit{v}+deu [$_{TopP}$ [uma maçã]  Top [$_{FocP}$ [para o Mario]  Foc  [$_{\textit{v}P}$ \textcolor{gray}{<A Olga> <\textit{v}+deu>} [$_{VP}$ \textcolor{gray}{<uma maçã> <deu> <para o Mario>} ] ] ] ] ]}\label{ex:cepeda:28b}
	\z
\z
