...
Like Inf-\textsc{dat}, \textit{la}{}-\textsc{dat} are sensitive to the AH, but the selectional properties of \textit{la} are not identical to those of the dative inflection. For instance, both types of datives are compatible with names of \textit{corporate bodies} \REF{ex:cornilescu:15}, but only Inf-\textsc{dat} are also felicitous with \textit{abstract} nouns, \textit{la}{}-\textsc{dat} are not \REF{ex:cornilescu:16}.
...
These facts follow from the analysis. A natural explanation for why a \textit{pe}{}-marked object can bind an IO (= \REF{ex:cornilescu:30a}) is that, in this case the IO stays low and \textit{may be (re)analyzed as a PP}, thus not competing with the DO.
